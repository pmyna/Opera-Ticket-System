...
class RegistrationForm(FlaskForm):
    ...
    zip_code = StringField('Postleitzahl', validators=[DataRequired(), Length(min=4, max=4, message='[4-stellig]'), Regexp(regex='^[0-9]{4}$', message='Nur Zahlen erlaubt')])
    ...
    landline = StringField('Festnetz', [Length(min=8, max=14, message='[Min 8 | Max 14]'), Regexp(regex='^[0-9]{8}(?:[0-9]+)?', message='Nur Zahlen erlaubt')])
    phone_number = StringField('Handy', [Length(min=8, max=14, message='[Min 8 | Max 14]'), Regexp(regex='^[0-9]{8}(?:[0-9]+)?', message='Nur Zahlen erlaubt')])
    password = PasswordField('Passwort', [DataRequired(), Length(min=5, max=20,  message='[Min 5 | Max 20]')])
